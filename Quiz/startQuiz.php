...
if (isset($_GET["requestQuiz"])) { //check if the user has started the quiz or not,if he did, then redirect him to restartQuiz
    $requestedQuizName = $_GET["requestQuiz"];
    $userID = $_SESSION['id'];
    $quizID = getQuizID($requestedQuizName);
    if (startedQuiz($userID, $quizID)) {
        header('location: restartQuiz.php?requestQuiz=' . $requestedQuizName);
    }
} else {
       ...
       var notShuffledQuestions = [];
        ...
        $(document).ready(function() {
            $('#startQuiz').click(function() {
                $.ajax({
                    url: 'quizController.php',
                    type: 'POST',
                    data: {
                        startedQuiz: document.getElementById("quizName").innerHTML
                    },
                    success: function(result) { //if error
                        if (result != "") {
                            window.location.reload();
                            alert("Error: quiz is already started");
                        }
                    }
                });
            });
        ...
        function checkAnswer(choice) {
            if (questions[questionIndex].answers[choice.id].correct) {
                ...
                points += 10*currentTime;
                var pts = document.getElementById("pts");
                pts.innerHTML = 'Points gained: <span class="font-weight-bold">' + points + 'pts</span>';
                nCorrect++;
                return true;
            } else {
        ...
        function endQuiz() {
            ...
            endTable.innerHTML = `<tbody>
                            ...
                            <tr>
                                <td>Total points</td>
                                <td class="text-primary">` + points + `pts</td>
                            </tr>
                        </tbody>`;
            submitResult();
        ...
        function submitResult() {
            $.ajax({
                ...
                type: 'POST',
                data: {
                    quizName: document.getElementById("quizName").innerHTML,
                    totalPoints: points,
                    totalWrong: nWrong,
        ...
        <div class="row">
            ...
            <div id=startBox class="startquiz-box col-md-4 offset-md-4 shadow text-center">
                <div class="jumbotron">
                    ...
                    <p>Answer a question correctly and you will get <b>10 points</b> for every second left in the timer!</p>
                    <h5>Good luck!</h5>
                    <hr>
                    <button id="startQuiz" name="startedQuiz" value="Game of thrones" class="btn btn-lg btn-success" onclick="startGame()">Start Quiz!</button>
            ...
            <div id=qBox class="hide quiz-box col-md-4 offset-md-4 shadow text-center">
                <div class="question-stats">
                    ...
                    <div id="pts" class="float-left text-info">Points gained: <span class="font-weight-bold">150pts</span>
                    </div>
                </div><br>
